...
QtObject {
    ...
    function parseHex(value: string): var {
        ...
        if (hex.length === 3) {
            return Qt.rgba(
                channel(parseInt(hex[0] + hex[0], 16)),
                channel(parseInt(hex[1] + hex[1], 16)),
                channel(parseInt(hex[2] + hex[2], 16)),
                1
            );
        }
        ...
        if (hex.length === 6 || hex.length === 8) {
            return Qt.rgba(
                ...
                hex.length === 8 ? channel(parseInt(hex.slice(6, 8), 16)) : 1
            );
        }
    ...
    function mix(color1, color2, mixRate: real): color {
        ...
        return Qt.rgba(
            first.r * inverse + second.r * rate,
            first.g * inverse + second.g * rate,
            first.b * inverse + second.b * rate,
            first.a * inverse + second.a * rate
        );
    }
}
