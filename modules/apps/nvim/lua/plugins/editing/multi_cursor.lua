                ...
                callback = function(args)
                    local function delete_key(key)
                        if in_vm_extend_mode() then
                            ...
                            vim.cmd((
                                "call b:VM_Selection.Edit.run_normal('%s', {'count': v:count1, 'recursive': 0, 'store': v:register, 'vimreg': 1})"
                            ):format(key))
                        end
                    end
                    ...
                    vim.keymap.set('n', 's', function()
                        if in_vm_extend_mode() then
                            ...
                            vim.cmd((
                                "call b:VM_Selection.Edit.run_normal('d%sl', {'recursive': 0, 'store': v:register, 'vimreg': 1})"
                            ):format(count))
                        end
